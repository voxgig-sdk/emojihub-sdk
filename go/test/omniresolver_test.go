...
import (
	...
	omni "github.com/voxgig-sdk/emojihub-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.EmojihubError); is {
				out := map[string]any{
					"name":    "EmojihubError",
...
func omniCtx(arg any, client *sdk.EmojihubSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.EmojihubSDK); is {
			client = live
