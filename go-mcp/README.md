...
[MCP](https://modelcontextprotocol.io) server exposing the Emojihub SDK as
two agent tools — `emojihub_list` and `emojihub_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/emojihub-mcp)
make build

# 2. Provide credentials via the environment
export EMOJIHUB_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user emojihub \
  -- /absolute/path/to/emojihub-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./emojihub-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// emojihub_list: first page of records
{ "entity": "all" }
{ "entity": "all", "query": { } }

// emojihub_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/emojihub-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user emojihub \
     -- "$PWD"/dist/*/emojihub-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `emojihub_list` and `emojihub_load` tools now appear
   in new sessions. Ask the agent to *"list all using emojihub"*
   and it calls `emojihub_list` with `{"entity":"all"}`.
...
```sh
export EMOJIHUB_APIKEY=sk_live_xxx            # API key
export EMOJIHUB_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `emojihub_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `emojihub_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `EMOJIHUB_APIKEY` | API key sent with every request. |
| `EMOJIHUB_BASE` | Optional override of the API base URL. |

### Entities

The 6 entities valid as the `entity` argument:

all | category | group | random | search | similar

### Smoke test via HTTP (raw JSON-RPC)

```sh
./emojihub-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"emojihub_load","arguments":{"entity":"category","query":{"id":1}}}}'
```
