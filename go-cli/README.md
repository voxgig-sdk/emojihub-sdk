...
boru-driven command-line client **and** interactive REPL for the Emojihub
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/emojihub-cli)
make build
...
./emojihub-cli list all
./emojihub-cli list category

# 5. Override the API base URL for a single call
EMOJIHUB_BASE=https://api.example.com ./emojihub-cli list all
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/emojihub-cli
   ```
...
Run with no arguments to open a REPL (prompt `emojihub>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/emojihub-cli`. |
...
(`./emojihub-cli <expr>`) and interactively (the REPL), and expressions compose the
