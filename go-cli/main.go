// Command Emojihub-cli is a CLI + REPL for the Emojihub SDK,
...
const prompt = "emojihub"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "all category group random search similar"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      EMOJIHUB_APIKEY (api key), EMOJIHUB_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: EMOJIHUB_APIKEY carries the API key and
	// EMOJIHUB_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("EMOJIHUB_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("EMOJIHUB_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewEmojihubSDK(opts)
