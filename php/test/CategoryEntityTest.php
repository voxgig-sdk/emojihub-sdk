...
class CategoryEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = EmojihubSDK::test($seed, null);
        $seen = iterator_to_array($base->Category(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = EmojihubConfig::shared_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = EmojihubSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set EMOJIHUB_TEST_CATEGORY_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function category_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("EMOJIHUB_TEST_CATEGORY_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["EMOJIHUB_TEST_CATEGORY_ENTID"]);
