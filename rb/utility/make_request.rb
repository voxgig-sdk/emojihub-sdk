...
module EmojihubUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = EmojihubResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = EmojihubResponse.new(fetched)
    else
