...
class SimilarEntity
  ...
  def data_set(args)
    if args
      @_data = EmojihubHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = EmojihubHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  #
  # @param reqmatch [SimilarLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Similar.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Similar, Hash] the loaded Similar; raises EmojihubError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = EmojihubHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
