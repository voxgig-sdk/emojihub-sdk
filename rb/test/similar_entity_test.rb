...
class SimilarEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set EMOJIHUB_TEST_SIMILAR_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    similar_ref01_data_dt0_loaded, err = similar_ref01_ent.load(similar_ref01_match_dt0, nil)
    assert_nil err
...
def similar_basic_setup(extra)
  ...
  entid_env_raw = ENV["EMOJIHUB_TEST_SIMILAR_ENTID"]
