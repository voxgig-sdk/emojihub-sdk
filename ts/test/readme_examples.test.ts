...
//     (\`new EmojihubSDK(...)\` / \`EmojihubSDK.test(...)\` become
//     \`EmojihubSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"all":{"test01":{"id":"test01"}},"category":{"test01":{"id":"test01"}},"group":{"test01":{"id":"test01"}},"random":{"test01":{"id":"test01"}},"search":{"test01":{"id":"test01"}},"similar":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new EmojihubSDK(...)\` and any
// \`EmojihubSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+EmojihubSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/EmojihubSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
