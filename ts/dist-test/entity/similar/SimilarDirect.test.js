...
(0, node_test_1.describe)('SimilarDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-similar', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-similar', setup.live))
            return;
...
function liveScenariosActive() { return false && process.env.EMOJIHUB_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.EmojihubSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
        }));
        let idmap = env['EMOJIHUB_TEST_SIMILAR_ENTID'];
    ...
    const client = new __1.EmojihubSDK({
        base: 'http://localhost:8080',
