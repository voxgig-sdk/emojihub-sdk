...
class FtCtrl extends EmojihubControl
{
...
class FtClient
{
    ...
    private EmojihubUtility $utility;

    public function __construct(EmojihubUtility $utility, array $options)
    {
    ...
    public function get_utility(): EmojihubUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (EmojihubContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public EmojihubContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (EmojihubContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = EmojihubConfig::make_feature($name);
    ...
    private static function buildUrl(EmojihubSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof EmojihubError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new EmojihubResponse($fetched);
        }
    ...
    private function fail(EmojihubContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (EmojihubContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
