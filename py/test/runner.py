# Emojihub SDK test runner

from __future__ import annotations
import os
import json

from emojihub_sdk.utility.voxgig_struct import voxgig_struct as vs


class EmojihubTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            EmojihubTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = EmojihubTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = EmojihubTestRunner.getenv("EMOJIHUB_TEST_LIVE")
        override = EmojihubTestRunner.getenv("EMOJIHUB_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = EmojihubTestRunner.getenv(key)
        ...
        explain = EmojihubTestRunner.getenv("EMOJIHUB_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["EMOJIHUB_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if EmojihubTestRunner._test_control is not None:
            return EmojihubTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                EmojihubTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            EmojihubTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = EmojihubTestRunner.load_test_control()
...
def load_env_local():
    EmojihubTestRunner.load_env_local()


def env_override(m):
    return EmojihubTestRunner.env_override(m)


def entity_data(v):
    return EmojihubTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return EmojihubTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return EmojihubTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return EmojihubTestRunner.load_test_control()


def live_client_options():
    return EmojihubTestRunner.live_client_options()


def live_delay_ms():
    return EmojihubTestRunner.live_delay_ms()
